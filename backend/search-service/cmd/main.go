...
import (
	...
	grpcHandler "github.com/voice-diary/backend/search-service/delivery/grpc"
...
func main() {
	...
	logger.Info("Starting search-service", zap.String("environment", cfg.Environment))

	// Initialize search service
	searchService, err := service.NewSearchService(cfg.ElasticsearchURL)
	if err != nil {
		logger.Fatal("Failed to initialize search service", zap.Error(err))
	}

	// Initialize gRPC handler
	handler := grpcHandler.NewSearchHandler(searchService)

	// Start gRPC server
	grpcServer := grpcutil.NewServer(cfg.GRPCPort)
	searchpb.RegisterSearchServiceServer(grpcServer.Server, handler)
